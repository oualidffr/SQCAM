...
class ECABlock(nn.Module):
    ...
    def __init__(self, channels, in_H=None, in_W=None, quantiles=None, gamma=2, b=1):
        super().__init__()
...
# ---------------- UNet with ECA ---------------- 
class SmallUNet_AllCA_vECA(nn.Module):
    ...
    def __init__(self, in_ch=3, base_ch=16, quantiles=[0.25, 0.5, 0.75, 0.95]):
        ...
        self.ca_e1 = ECABlock(base_ch, in_H=192, in_W=256, quantiles=q)
        self.pool1 = nn.MaxPool2d(2)
        self.enc2 = DoubleCGR(base_ch, base_ch*2)
        self.ca_e2 = ECABlock(base_ch*2, in_H=96, in_W=128, quantiles=q)
        self.pool2 = nn.MaxPool2d(2)
        self.enc3 = DoubleCGR(base_ch*2, base_ch*4)
        self.ca_e3 = ECABlock(base_ch*4, in_H=48, in_W=64, quantiles=q)
        self.pool3 = nn.MaxPool2d(2)
        self.enc4 = DoubleCGR(base_ch*4, base_ch*8)
        self.ca_e4 = ECABlock(base_ch*8, in_H=24, in_W=32, quantiles=q)
        self.pool4 = nn.MaxPool2d(2)

        # Bottleneck
        self.bottleneck = DoubleCGR(base_ch*8, base_ch*16)
        self.ca_bottleneck = ECABlock(base_ch*16, in_H=12, in_W=16, quantiles=q)

        # Decoder
        self.up4 = nn.ConvTranspose2d(base_ch*16, base_ch*8, 2, stride=2)
        self.dec4 = DoubleCGR(base_ch*16, base_ch*8)
        self.ca_d4 = ECABlock(base_ch*8, in_H=24, in_W=32, quantiles=q)
        self.up3 = nn.ConvTranspose2d(base_ch*8, base_ch*4, 2, stride=2)
        self.dec3 = DoubleCGR(base_ch*8, base_ch*4)
        self.ca_d3 = ECABlock(base_ch*4, in_H=48, in_W=64, quantiles=q)
        self.up2 = nn.ConvTranspose2d(base_ch*4, base_ch*2, 2, stride=2)
        self.dec2 = DoubleCGR(base_ch*4, base_ch*2)
        self.ca_d2 = ECABlock(base_ch*2, in_H=96, in_W=128, quantiles=q)
        self.up1 = nn.ConvTranspose2d(base_ch*2, base_ch, 2, stride=2)
        self.dec1 = DoubleCGR(base_ch*2, base_ch)
        self.ca_d1 = ECABlock(base_ch, in_H=192, in_W=256, quantiles=q)
